...
const FlightsList = () => {
    ...
    function destinationListFunction() {
        return(
            <div>
                {destinations.map(currentdestination=>{
                    return <Destination destination={currentdestination} key={currentdestination._id} />;
                })}
            </div>
        )
    }
    setDestinationList(destinationListFunction());

    (function () {
        return(axios.get('http://localhost:3001/destinations/')
        .then(res => {
